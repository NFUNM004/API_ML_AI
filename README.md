...
### [20x20ppt 点击下载](https://gitee.com/NFUNM004/api_hanyutong_app/blob/master/%E6%B1%89%E8%AF%AD%E9%80%9Aapp.pptx)

### 价值主张：
第二语言学习时需要语言理论知识的指导，对语法规则进行理解和把握。但汉语往往以意合为主，语法关系丰富，表意灵活。正是因为这个原因，外国人在学习汉语的过程中常常有弄不清句子语法结构和词语词性，能说但稀里糊涂的问题。市面上的汉语学习APP大多都是注重汉语拼音的学习，对句型语法的学习较少。汉语通APP通过分析句子的的句法结构信息，助于用户分辨句子结构，助力他们更好地进行汉语学习。
...
### 加值宣言：
本APP运用人工智能自然语言处理技术，通过词法分析识别出文本串中的词性，帮助用户辨别词语词性；通过依存句法分析分析句子的句法结构信息，有助于用户分辨句子结构；若用户不想打字，能通过通用文字识别技术，识别用户上传的图像中的文本，实现不打字也能输入长文本。
...
- 通用文本识别识别的准确率能达到95%。但文字识别可能受图片清晰度和图片中字体清晰度、字体样式、简繁体等影响，导致识别出错。
...
### 交互及界面、信息设计
- 词性分析运用了词法分析API，对句子进行自动分词，判断句子中的词语词性，句法分析运用了依存句法分析API，判断句子中词语间的关系。
- 拍照识别使用了通用文字识别API,实现识别图像中的文本。

### 口头操作说明
本产品产品的主要功能分为“词性分析”、“句法分析”和“练习”。用户选择词性分析后输入文本，若输入词语则反馈词语词性，若输入句子则系统自动划分句子中的词语，并输出分析结果。句法分析系统自动划分句子的成分，并显示句子的结构和词于词之间的关系。用户可于“练习”的“笔记本”查看自己收集的词性、句法分析和习题，也可选词语、短句、长文本等不同练习难度，对词法、句子结构的判定进行练习，提升自己的汉语水平。  
...
#### 词法识别API：
- [百度词法分析API官方文档](https://cloud.baidu.com/doc/NLP/s/Ck3pmn1l6#%E8%AF%8D%E6%B3%95%E5%88%86%E6%9E%90)
- [讯飞词法分析API官方文档](https://www.xfyun.cn/doc/nlp/lexicalAnalysis/API.html#%E6%8E%A5%E5%8F%A3%E8%AF%B4%E6%98%8E)
- 百度词法分析API调用代码:
```python
...
#### 依存句法分析API：
- [百度依存句法分析API官方文档](https://cloud.baidu.com/doc/NLP/s/Ck3pmn1l6#%E4%BE%9D%E5%AD%98%E5%8F%A5%E6%B3%95%E5%88%86%E6%9E%90)
- [讯飞依存句法分析API官方文档](https://www.xfyun.cn/doc/nlp/dependencyParsing/API.html)
...
### API使用比较分析
|          | 百度依存句法分析API                                                                               |                 讯飞依存句法分析API                                          |
| -------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | 
| 成熟程度 | 算法通用性强，句法结构的描述体系简洁通用，在大规模人工标注的树库的基础上，借助海量知识与弱标记数据等语义信息训练，有效缓解语言分析中的结构歧义问题 |基于哈工社会计算与信息检索研究中心历时十年研制的一整套开放中文自然语言处理系统，无需下载 SDK，提供WEB API接口，可进行跨平台调用，接入方便  |
| 价格     | 可免费调用50w次，5QPS/天；付费后月租6000元/QPS；日租280元/QPS               |   免费调用：每日2w次服务量限额，提升限额需进行收费        |
